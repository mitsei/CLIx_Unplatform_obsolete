...
class Player extends Phaser.Sprite {
    ...
    move() {
        ...
        this.movement = setTimeout(function() {
            _this.body.velocity.x = distance * _this.speed / 60
    ...
    labelDelay(label) {
        ...
        if (game.level == 4) {
            this.flair = game.add.sprite(this.x - 128 + 18, this.y + 16,'speedometer')
        } else {
            this.flair = game.add.sprite(this.x - 128+ 18, this.y + 16,'hourglass')
        }
        
        this.flair.scale.set(.8)
        
        this.sprite = game.add.sprite(this.x - 128 + 18, this.y + 40,'minicat')
...
class Computer extends Player {
    ...
    labelSpeed(label) {
        ...
        this.sprite = game.add.sprite(this.x - 128 + 18, this.y + 40,'minimouse')
        this.sprite.scale.set(1.3)
        
        this.flair = game.add.sprite(this.x - 128 + 18, this.y + 16,'speedometer')
        this.flair.scale.set(.8)
                
        this.text = game.add.text(this.x - 96 + 18, this.y + 16, "Speed:\n" + label + " m/s", {
            font: "18px PT Mono",
            fill: "#000",
            align: "left"
		});        
...
class Lever extends Phaser.Sprite {
    constructor(location, player) {
        ...
        this.textcolor = '#fff'
        this.text = game.add.text(this.slider.x - 10, this.slider.y + 128, "Speed:\n " + player.speed + " m/s", {
            font: "28px PT Mono",
            fill: this.textcolor,
            align: "left"
		});
...
class BetButton extends Phaser.Sprite {
    ...
    check(comp,player) {
        ...
        if (Math.abs(player.x - comp.x) < 32) { // on time
            game.ontime = true
            return 2; // the frame it should be
        } else if (player.x < comp.x) { // too slow
            game.slow = true
            return 1;
        } else { // too fast
            game.fast = true
...
class Prompt extends TextBox{
    constructor(x,y) {
        ...
        this.indicator = new ProgressIndicator(x+200,y)
    ...
    makeGoButton(x,y) {
        ...
        this.go_button.wiggle = function() {    
            var _this = this
                // console.log(this)
    ...
    resetGame() { //reset delay too asdf
        ...
        clearInterval(promptBox.go_button.wiggler)
...
class DelayController extends TextBox {
    constructor(x,y) {
        ...
        this.tint = '0xc04a6c'
        this.minicat = game.add.sprite(x+35, y-40, 'minicat')
        ...
        this.minus.events.onInputUp.add(function(){
            playertwo.index = playertwo.delays.indexOf(playertwo.delay)
            if (playertwo.index  <= 0) {
                playertwo.delay = playertwo.delays[playertwo.delays.length -1]
            } else {          
                playertwo.delay = playertwo.delays[(playertwo.index - 1) % playertwo.delays.length ]
            }
            _this.delayDisplay.text.text.destroy()
        ...
        // this.sprite.scale.set(1.3)
        this.text = game.add.text(playertwo.x - 96, playertwo.y + 16, "Speed:\n" + playertwo.speed + " m/s", {
            font: "18px PT Mono",
            fill: "#000",
            align: "left"
		});
